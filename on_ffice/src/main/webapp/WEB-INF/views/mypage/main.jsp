...
<main>
    <div class="row">
        ...
        <h2 onclick="showHide">나의 TODO 목록</h2>
		...
		<hr>
        	...
        	<table id=todoList class="text-center table table-hover text-end">
				<thead>
				<tr>
				<th>업무번호</th>
				<th>담당자</th>
        ...
        <span class="input-group-text">넘겨줄 작업 제목</span>
  		<select class="form-select btn btn-secondar" name="todo_num">
  			<option selected>넘겨줄 작업을 선택해주세요</option>
  				<c:forEach items="${todoList}" var="todo">
  				<option value="${todo.todo_num}">작업번호[${todo.todo_num}] 작업제목 : ${todo.todo_title} 담당자 : ${todo.checker }</option>
  				</c:forEach>
				</select>
				<br/>
	  			<span class="input-group-text">작업내용</span>
